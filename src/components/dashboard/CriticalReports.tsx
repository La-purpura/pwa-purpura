...
export function CriticalReports() {
    ...
    useEffect(() => {
        const fetchCriticalReports = async () => {
            try {
                const res = await fetch('/api/reports?priority=CRITICAL&priority=HIGH&limit=3');
                if (res.ok) {
        ...
        return (
            <div className="bg-white dark:bg-[#1a1a1a] p-8 rounded-[2.5rem] border border-gray-100 dark:border-gray-800 shadow-sm">
                <div className="flex items-center justify-between mb-8">
                    <h3 className="text-lg font-black uppercase tracking-tight">Reportes de Prioridad</h3>
                </div>
                            ...
                            <div className="flex items-start justify-between gap-4">
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2 mb-2">
                                        <span className={`px-2 py-0.5 rounded-lg text-[9px] font-black uppercase ${report.priority === 'CRITICAL'
