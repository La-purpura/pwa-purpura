...
export default function AdminDashboardMobile() {
    ...
    useEffect(() => {
        ...
        // Fetch Recent Activity (Requests)
        fetch('/api/requests')
            .then(res => res.json())
            .then(data => {
                const pending = data.filter((r: any) => r.status === 'pending').slice(0, 3);
                setRequests(pending);
            })
            .catch(console.error);
    }, []);
        ...
        <div className="space-y-6 pb-24 px-1">
            ...
            <section className="grid grid-cols-2 gap-3">
                ...
                {hasPermission("users:invite") && (
                    <Link href="/team" className="p-4 bg-orange-50 dark:bg-orange-900/10 border border-orange-200 dark:border-orange-800 rounded-xl flex items-center gap-3 active:scale-95 transition-transform">
